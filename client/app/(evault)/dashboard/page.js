...
import { FaListAlt, FaPlus, FaUserCircle } from "react-icons/fa";
			...
			<div className="flex flex-col lg:flex-row w-full">
				...
				<div className="flex-1 p-5">
					<div className="flex justify-between items-center gap-5 mt-5">
						<h3 className="text-3xl font-bold text-blue-900">Your Files</h3>
						<button
							type="button"
							onClick={toggleTileView}
							className="p-2 rounded-full hover:bg-white/50 border"
							title={tileView ? "List View" : "Grid View"}
						>
							{tileView ? (
								<FaListAlt className="text-blue-900 text-xl" />
							) : (
								<LuGrid className="text-blue-900 text-xl" />
							)}
						</button>
					</div>
					<div
						className={`flex flex-wrap items-center justify-center md:justify-normal mt-5 ${
							tileView ? "gap-5" : "rounded"
						}`}
					>
						{!tileView && (
							<div className="w-full items-center gap-2 p-3 bg-white/50 transition border-b-2 relative hidden lg:flex">
								<div className="flex-1 flex flex-col lg:flex-row lg:gap-5 lg:mr-5">
									...
									<p className="font-bold text-sm md:text-base text-blue-950 ">
										Last Modified
									</p>
								</div>
								<button
						...
						{files.map((fname, index) => {
							return tileView ? (
								<FileCard key={fname} filename={fname} />
							) : (
								<FileListItem
									key={fname}
									filename={fname}
				...
				<div className="fixed right-0 bottom-0 -translate-x-1/2 -translate-y-1/2">
					...
					<input id="dashboard-add-file" type="file" className="hidden" />
				</div>
			</div>
		</>
	);
